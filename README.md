## Hi there 👋

I'm Jake, a programmer and Computer Science student.

I mostly use Python (and prefer to use it over other languages), but have a fair amount of experience programming on the web (full-stack) - next.js, HTML/CSS/JS, express, SQL.

<details open>
<summary>
  <h2>
    What I'm working on
  </h2>
</summary>

### Projects
- Final Project at UA92 - AI-powered fact-checking & credibility analysis software (for the contemporary issue of misinformation/disinformation on the web).
### Learning
- Intermediate-Advanced Data Structures & Algorithms.
- React
- Next.js
- TailwindCSS
- TypeScript

</details>


<details open>

<summary>
  <h2>
    What's next?
  </h2>
</summary>

### Planning to learn/study
- Software Design fundamentals and patterns.
- Secure Software Development.
- Low-level Programming.
- Operating Systems.
- Embedded Programming.
- Object-oriented Software Design.
- Cyber Security - especially pen-testing and red-teaming.
- Game Development (Unity?).
- Game Modding.
- Mobile development (Swift).
- C#,
- Java,
- Rust,
- golang.

### Future project ideas
- AI-assisted learning platform for those with ADHD or similar conditions.

</details>


<details>
<summary>
  <h2>
    Everything else (the non-professional stuff)
  </h2>
</summary>
  
### Music
- I listen to metalcore/deathcore music - Polaris, Currents, Invent Animate, Lorna Shore, Whitechapel, Shadow of Intent, LANDMVRKS.
- I've been playing the guitar for about 4 years (since August 2021).
- Interested in learning how to play the drums.

### Games
#### Currently Playing: `Hollow Knight: Silksong (Act 3)`
#### Favourites
- Dark Souls & Soulslikes.
- Hollow Knight.
- Roguelikes - Balatro, Risk of Rain 2.
#### Games to play next
- Nine Sols.
- Celeste.
- Clair Obscur: Expedition 33.
- Ghost of Tsushima.
- Ghost of Yōtei.
- Megabonk.
- Outer Wilds.
- Black Myth: Wukong.
- WUCHANG: Fallen Feathers.

</details>
